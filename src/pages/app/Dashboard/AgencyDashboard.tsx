...
const AgencyDashboard = () => {
  ...
  const mostUsedFeatures = [
    { feature: 'Posting', usage: 45, color: 'bg-blue-500' },
    { feature: 'Email Marketing', usage: 30, color: 'bg-green-500' },
    { feature: 'AI Content Generation', usage: 15, color: 'bg-purple-500' },
    { feature: 'Analytics', usage: 10, color: 'bg-yellow-500' },
  ];
  ...
  const quickLinks = [
    ...
    { name: 'Billing', icon: DollarSign, color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'hover:border-yellow-200', href: '/billing-subscription' },
    { name: 'Queries', icon: MessageSquare, color: 'text-purple-600', bg: 'bg-purple-50', border: 'hover:border-purple-200', href: '/queries-tickets' },
    { name: 'Announcements', icon: Globe, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'hover:border-indigo-200', href: '/announcements' },
    { name: 'Training', icon: PenSquare, color: 'text-red-600', bg: 'bg-red-50', border: 'hover:border-red-200', href: '/training' },
    { name: 'Manage Users', icon: Users, color: 'text-blue-600', bg: 'bg-blue-50', border: 'hover:border-blue-200', href: '/manage-users' },
    { name: 'Email Marketing', icon: Mail, color: 'text-green-600', bg: 'bg-green-50', border: 'hover:border-green-200', href: '/email-marketing' },
    { name: 'Posting', icon: PenSquare, color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'hover:border-yellow-200', href: '/posting' },
    { name: 'SMS Marketing', icon: Phone, color: 'text-purple-600', bg: 'bg-purple-50', border: 'hover:border-purple-200', href: '/sms-marketing' },
    { name: 'WhatsApp', icon: MessageSquare, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'hover:border-indigo-200', href: '/whatsapp-messaging' },
    { name: 'AI Generation', icon: Settings, color: 'text-red-600', bg: 'bg-red-50', border: 'hover:border-red-200', href: '/ai-content-generation' },
  ];
  ...
  return (
    <div className="flex h-screen bg-gray-50/50 font-sans text-gray-900 overflow-hidden">
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <motion.div variants={itemVariants}>
                ...
                <p className="text-gray-500 mt-1">Manage your brands and campaigns.</p>
              </motion.div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { title: 'Total Brands', value: totalBrands, sub: 'Managed by agency', icon: Building2, color: 'blue' },
                { title: 'Total Users', value: totalUsers, sub: `Including ${agencyStaff} staff`, icon: Users, color: 'green' },
                { title: 'Total Earnings', value: `$${totalEarnings.toLocaleString()}`, sub: 'Agency revenue', icon: DollarSign, color: 'yellow' },
                { title: 'Messages Sent', value: totalEmails + totalSMS + totalWhatsApp, sub: `${totalEmails} emails | ${totalSMS} SMS`, icon: Mail, color: 'purple' },
              ].map((stat, index) => (
                <motion.div
                  key={index}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-gray-100 text-gray-400 text-xs uppercase tracking-wider">
                        ...
                        <th className="py-4 font-semibold">Assigned To</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {upcomingTasks.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="py-4 text-gray-600">{item.assigned}</td>
                        </motion.tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <motion.div variants={itemVariants} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
                <div className="flex items-center mb-8 gap-3">
                  ...
                  <h2 className="text-lg font-bold text-gray-900">Most Used Features</h2>
                </div>
      ...
      {/* Overlay for mobile sidebar */}      <AnimatePresence>
        {sidebarOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
